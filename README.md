...
- [x] 数据导入 / 导出（JSON），支持跨设备迁移 — v0.3.0
- [x] 周视图内拖入后自动吸附到 30 分钟边界的视觉预览 — v0.4.0
- [x] 键盘快捷键系统 — v0.4.0
- [x] 任务搜索 & 筛选 — v0.4.0
- [x] 暗色主题（浅色 / 暗色 / 跟随系统）— v0.4.0
- [x] 存储层版本化 + 迁移机制 + 容量监控（数据安全加固）— v0.5.0
- [x] Vitest 单元测试覆盖（types / utils / store）— v0.6.0
- [x] Playwright E2E：任务创建 → 拆分 → 拖拽排期 → 完成 → 计时全链路 — v0.6.0
- [x] CI 流水线（lint + 单测 + 构建 + E2E）— v0.6.0
- [x] 主题色自定义面板（预设色板 + 自定义取色器，暗色主题跟随）— v0.7.0
- [x] 拖拽手感动画（浮层缩放跟随、落位弹性、吸附预览过渡）— v0.7.0
